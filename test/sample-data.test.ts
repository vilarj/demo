...
function randomISODate(past = false): ISO8601Date {
  const date = past ? faker.date.recent({ days: 30 }) : faker.date.soon({ days: 180 });
  return date.toISOString().split('T')[0] as ISO8601Date;
}

function createEmployees(count: number): Record<EmployeeId, Employee> {
  const idPool = faker.helpers.shuffle(Array.from({ length: 6000 - 2000 + 1 }, (_, i) => 2000 + i));
...
function createTools(count: number, employeeIds: EmployeeId[]): Record<ToolId, Tool> {
  const tools: Record<ToolId, Tool> = {};
  const snPool = faker.helpers.shuffle(
    Array.from({ length: count }, (_, __) => `SN${faker.string.alphanumeric(10).toUpperCase()}`),
  );
  for (let i = 1; i <= count; i += 1) {
    const id = `T${i}` as ToolId;
    const assigned = faker.datatype.boolean();
    const assignedTo = assigned ? faker.helpers.arrayElement(employeeIds) : null;
...
describe('MockInventorySystem population', () => {
  it.skip('should create a system with 100 employees and 500 tools', () => {
    ...
    fs.writeFileSync(path.join(__dirname, '../src/data/tools.json'), JSON.stringify(tools, null, 2));
  });
});
